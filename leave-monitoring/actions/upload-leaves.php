...
if (isset($input['records']) && is_array($input['records'])) {
    ...
    if (!empty($ext_dob_pob) && strpos($ext_dob_pob, ' / ') !== false) {
        ...
        if ($dStr !== 'N/A') {
            $ts = strtotime($dStr);
            if ($ts)
                $current_employee_dob = date('Y-m-d', $ts);
        }
        if ($pStr !== 'N/A')
            $current_employee_pob = $pStr;
    }
        ...
        foreach ($records as $index => $data) {
            ...
            if ($index < 15) {
                foreach ($data as $idx => $cell) {
                    ...
                    if (stripos($cell, 'REASON') !== false || stripos($cell, 'PARTICULAR') !== false) {
                        $reason_col = $idx;
                    }
                    if (stripos($cell, 'STATION') !== false)
                        $station_col = $idx;
                    if (stripos($cell, 'PAY') !== false && stripos($cell, 'STATUS') !== false)
                        $pay_status_col = $idx;
                    if (stripos($cell, 'DAYS') !== false)
                        $total_days_col = $idx;
                    if (stripos($cell, 'REMARKS') !== false) {
                        $remarks_col = $idx;
                        if ($reason_col === null)
                            $reason_col = $idx; // Fallback
                    }
                    ...
                    foreach ($name_keywords as $kw) {
                        ...
                        if ($kw === 'NAME' || $kw === 'SURNAME' || $kw === 'EMPLOYEE') {
                            if ($cell_txt === $kw || strpos($cell_txt, $kw . ':') !== false)
                                $match = true;
                        } else {
                            if (strpos($cell_txt, $kw) !== false)
                                $match = true;
                        }
                        ...
                        foreach ($b_keywords as $bkw) {
                            if (strpos($cell_txt, $bkw) !== false) {
                                for ($off = 1; $off <= 11; $off++) {
                                    ...
                                    if (empty($bval) || strlen($bval) < 2 || strpos($bval, '(') !== false || preg_match('/SURNAME|FIRST|NAME|CHECKED|FROM|VERIFIED|BIRTH/i', $bval))
                                        continue;
                ...
                for ($i = 0; $i < $col_count; $i++) {
                    ...
                    if (empty($val) || $val == 'x' || $val == 'X' || strlen($val) < 2)
                        continue;
                ...
                if ($period_from) {
                    ...
                    if (empty($reasonRaw) || is_numeric($reasonRaw))
                        $reasonRaw = trim(strval($data[$base + 2] ?? ''));

                    $reason = $mapShortcutToFull($reasonRaw);

                    // Look for station (could be at base+2 or base+3)
                    $station = trim(strval($data[$base + 3] ?? ''));
                    if (empty($station) || $station == $reasonRaw)
                        $station = trim(strval($data[$base + 4] ?? ''));
            ...
            if ($error_count > 0)
                $msg .= " File: [ $filename ] | $error_count errors. " . implode(" | ", array_slice($error_details, 0, 1));
            if ($success_count == 0 && $error_count == 0) {
                $msg = "No records found in: $filename";
                if ($skipped_count > 0)
                    $msg .= " (Skipped $skipped_count rows that lacked dates or reason).";
            }
        }
    } catch (Throwable $e) {
