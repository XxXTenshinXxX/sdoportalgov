...
<div class="topbar">
    ...
    if (isset($_SESSION['user_id'])) {
        ...
        // Fetch unread activity logs count (since last view). Admins see everything, others exclude admin activity.
        $logCount = 0;
        $roleFilter = ($_SESSION['role'] === 'admin') ? "" : "JOIN users u ON l.user_id = u.id WHERE u.role != 'admin' AND ";
        $querySuffix = ($_SESSION['role'] === 'admin') ? "l.created_at > (SELECT IFNULL(last_viewed_logs, '1970-01-01 00:00:00') FROM users WHERE id = $uId)" : "l.created_at > (SELECT IFNULL(last_viewed_logs, '1970-01-01 00:00:00') FROM users WHERE id = $uId)";
        // Simplified query for admin:
        ...
        if ($logRes) {
            $logCount = $logRes->fetch_assoc()['total'];
        }
    }
    ?>
    <div class="d-flex align-items-center">
        <div class="dropdown me-3">
            <div class="position-relative cursor-pointer" id="notificationsDropdown" data-bs-toggle="dropdown"
                ...
                <span id="unread-notification-badge" class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger <?php echo $unreadCount > 0 ? '' : 'd-none'; ?>"
            ...
            <div class="dropdown-menu dropdown-menu-end shadow-lg border-0 py-0 overflow-hidden"
                ...
                <div id="dropdown-notification-list" class="notification-list" style="max-height: 300px; overflow-y: auto;">
        ...
        <div class="dropdown me-3">
            <div class="position-relative cursor-pointer" id="settingsDropdown" data-bs-toggle="dropdown" aria-expanded="false"
                ...
                <span id="activity-log-dot" class="position-absolute top-0 start-100 translate-middle p-1 bg-danger border border-light rounded-circle <?php echo $logCount > 0 ? '' : 'd-none'; ?>" style="margin-top: 6px; margin-left: -6px;">
            ...
            <ul class="dropdown-menu dropdown-menu-end shadow border-0" aria-labelledby="settingsDropdown"
                ...
                <li><a class="dropdown-item py-2" href="activity_logs.php">
                        <i class="bi bi-clock-history me-2"></i> Activity Logs
                        <span id="activity-log-badge" class="badge rounded-pill bg-danger ms-1 <?php echo $logCount > 0 ? '' : 'd-none'; ?>" style="font-size: 0.65rem;">
                            <?php echo $logCount; ?>
                        </span>
                    </a></li>
            </ul>
