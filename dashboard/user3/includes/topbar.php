...
<div class="topbar">
    ...
    if (isset($_SESSION['user_id'])) {
        ...
        // Fetch unread activity logs count (since last view), excluding admin activity
        $logCount = 0;
        $logRes = $conn->query("SELECT COUNT(*) as total FROM activity_logs l JOIN users u ON l.user_id = u.id WHERE u.role != 'admin' AND l.created_at > (SELECT IFNULL(last_viewed_logs, '1970-01-01 00:00:00') FROM users WHERE id = $uId)");
        if ($logRes) {
